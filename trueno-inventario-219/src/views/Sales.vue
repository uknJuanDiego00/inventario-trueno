<template>
  <div class="page-container">
            ...
            <div class="card-top-row">
              ...
              >
                ...
                <template v-else-if="product.stock <= (product.minStock || 5)">
                  <AlertTriangle :size="12" /> Stock bajo ({{ product.stock }})
                </template>
                <template v-else>
                  <Check :size="12" /> {{ product.stock }} en stock
                </template>
              </span>
      ...
      <div class="pos-ticket-col" :class="{ 'hidden-on-mobile': mobileTab === 'catalog' }">
        <div class="ticket-card card">
          ...
          <div v-if="cart.length > 0" class="ticket-footer">
            ...
            <!-- Selección de Cliente (Sin Documento) -->
            <div class="client-selection-section">
              ...
              <select v-model="selectedClientId" class="form-control client-select">
            ...
            <!-- Sección Crédito: Anticipo y saldo pendiente -->
            ...
            >
              ...
              <div class="confirm-text-col">
                ...
                <span class="confirm-sub-text">{{ formatCurrency(cartTotal) }} • {{ selectedPaymentMethod || 'Selecciona pago' }}</span>
              </div>
            </button>
          </div>
        </div>
    ...
    <!-- ================= MODAL DE REGISTRO RÁPIDO DE CLIENTE (SIN DOCUMENTO) ================= -->
...
import {
  ...
  Smartphone, Building2, CreditCard, Clock
} from '@lucide/vue'
...
const quickClient = ref({ name: '', phone: '', notes: '' })
...
const creditPendingBalance = computed(() => {
  ...
  return Math.max(0, cartTotal.value - adv)
})
...
// ================= REGISTRO RÁPIDO DE CLIENTE (SIN DOCUMENTO) =================
function saveQuickClient() {
  ...
  quickClient.value = { name: '', phone: '', notes: '' }
  store.notify(`Cliente ${newC.name} creado y seleccionado`, 'success')
}
...
function submitSale() {
  ...
  const paidAmount = isCredit ? Math.max(0, Number(creditAdvance.value) || 0) : cartTotal.value
